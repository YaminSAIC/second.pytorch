...
from second.data.preprocess_saic import _read_and_prep_v9_saic
...
class SaicDataset(KittiDataset):
    def __init__(self, info_path, root_path, num_point_features,
                 target_assigner, feature_map_size, prep_func):
        super(SaicDataset, self).__init__(info_path, root_path, num_point_features,
                 target_assigner, feature_map_size, prep_func)

    def __getitem__(self, idx):
        return _read_and_prep_v9_saic(
            info=self._kitti_infos[idx],
            root_path=self._root_path,
            num_point_features=self._num_point_features,
            prep_func=self._prep_func)
