...
def prep_pointcloud_saic(input_dict,
                    root_path,
                    voxel_generator,
                    target_assigner,
                    db_sampler=None,
    ...
    # remove unwanted classes such as truck and so on
    ...
    if training:
        ...
        # already to second lidar axis!!!
        # gt_boxes = box_np_ops.box_camera_to_lidar(gt_boxes, rect, Trv2c)
        gt_boxes = saic.saic_box_cam_to_second_box_lidar(gt_boxes)
...
def _read_and_prep_v9_saic(info, root_path, num_point_features, prep_func):
